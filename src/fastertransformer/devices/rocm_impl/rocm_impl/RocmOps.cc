...
#include "src/fastertransformer/devices/rocm_impl/Dispatch.h"
...
void RocmDevice::copy(const CopyParams& params) {
    ...
    hipMemcpyAsync(dst.data(), src.data(), src.sizeBytes(), copyType, stream_);
    hipDeviceSynchronize();
    hipError_t result = hipGetLastError();
    if (result) {
        throw std::runtime_error(std::string("[FT][ERROR] Rocm runtime error: ") + (hipGetErrorName(result)) + " \n");
    }
...
TransposeOutput RocmDevice::transpose(const TransposeParams& params) {
    ...
    DISPATCH_ROCM_FUNCTION_GENERAL_TYPE(data_type, invokeTransposeAxis01,
...
SPECIALIZE_CAST_TO(int64_t, hip_bfloat16);
SPECIALIZE_CAST_TO(uint64_t, hip_bfloat16);
SPECIALIZE_CAST_TO(__half, int64_t);
SPECIALIZE_CAST_TO(__half, uint64_t);
SPECIALIZE_CAST_TO(hip_bfloat16, int64_t);
SPECIALIZE_CAST_TO(hip_bfloat16, uint64_t);
SPECIALIZE_CAST_TO(hip_bfloat16, __half);
SPECIALIZE_CAST_TO(__half, hip_bfloat16);
...
ConvertOutput RocmDevice::convert(const ConvertParams& params) {
    const auto& input = params.input;
